                ...
                <div class="display-row-flex" style="width: 100%;margin: 2px auto;">
                  ...
                  <div style="display: block;width: calc(100% - 30px);color: #232323;font-family: Muli SemiBold;font-size: 16px;line-height: 14px;padding-left: 10px;">
                    <div class="display-column-flex" style="width: 100%;">
                      <!-- v-if="testQuestion.answerType === 'text'" -->
                      ...
                      <!-- v-if="testQuestion.answerType === 'QUICK_REPLY'" -->
                      <div  class="display-column-flex">
                        <!--{{testQuestion}}-->
                        <!-- v-if="testQuestion.multipleAnswersAllowed"  -->
                        <div class="display-column-flex">
                          <!-- v-for="(item, index) in testQuestion.quickReplies" -->
                          <div class="display-row-flex"  style="margin: 5px auto;width: 100%;">
                            <div style="display: block;overflow-x: hidden;overflow-y: visible;width: 20px;">
                              <!-- v-if="testQuestion.quickReplies[index].checked === false" -->
                              <!-- v-on:change="answerChecked(index, testQuestion.quickReplies[index].checked)" -->
                              <input class="form-control" type="checkbox"

                              style="height: 16px;width: 16px;min-width: 16px;min-height: 16px;"
                              />
                              <!-- v-if="testQuestion.quickReplies[index].checked === true" -->
                              <!-- v-on:change="answerChecked(index, testQuestion.quickReplies[index].checked)" -->
                              <input class="form-control" type="checkbox"
                        ...
                        <div  class="">
                          <!-- v-for="(item, index) in testQuestion.quickReplies" -->
                          <div class="row"  style="margin: 20px auto;width: 100%;">
                            <div class="col-lg-1" style="padding-right: 0px; margin-top:3px;">
                              <!-- v-model="testQuestion.answer.quickReply" -->
                  ...
                  <div v-if="testCollection !== null && testOngoing === true" style="width: 100%;border-radius: 0 0 4px 4px;background-color: rgba(247,247,247,0.54);padding-left: 3%;padding-right: 3%;">
                    ...
                    <div class="" style="width: 50%;">
                      <div class=""><!--disableNext === false-->
                        <!-- v-if="testQuestion.qno !== testCollection.totalQuestions" -->
                        <button
...
<!-- v-if="testQuestion.qno === testCollection.totalQuestions" -->
<!-- v-on:click="completeTest(testCollection.testName)" -->
                      <button
                      :disabled="disableNext"
...
export default {
  ...
  mounted () {
    ...
    //     .doc('candidate').collection(self.$route.params.id)
    //     .doc('tests').collection(self.testId).doc('status').get()
    //     .then((docList) => {
    //       self.pauseTime = docList.data().pausedTime
    //       self.elementIsReady = true
    //     }).catch((error) => {
    //       console.log(error)
    //     })
  ...
  methods: {
    ...
    loadPreviousQuestion (questionNo) {
      // firestore.collection('users').doc('candidate').collection(this.uid).doc('tests').collection(this.testId).doc('data')
    ...
    //   }
    //   if (testName === '18REST Assessment') {
    //     Promise.all([this.updateQuestion(answer), this.updateTestCollection()])
    //       .then(() => {
    //         Promise.resolve(firebaseAuth.currentUser.getIdToken().then((token) => {
    //           // console.log('Token instance', token)
    //           var instance = axios.create({
    //             // baseURL: 'https://813d3bac.ngrok.io',
    //             baseURL: appengineBaseurl,
    //             headers: {
    //               'Access-Control-Allow-Origin': '*',
    //               'Authorization': 'Bearer ' + token
    //             }
    //           })
    //           Promise.resolve(instance.post('/v1/candidate/test/18RESTAssessment/').then((response) => {
    //             if (response.error) {
    //               // this.loaderEnabled = true
    //             } else {
    //               this.testCollection.testStatus = 1
    //               this.testStatus = this.testCollection.testStatus
    //               eventBus.$emit('stopTest', this.testStatus)
    //               this.testOngoing = false
    //               // this.loaderEnabled = true
    //               this.$router.push('/Candidate/Tests/' + this.$route.params.id)
    //             }
    //           }).catch((error) => {
    ...
    //             }
    //           })
    //           Promise.resolve(instance.post('/v1/candidate/test/bigFivePersonalityTest/').then((response) => {
    //             if (response.error) {
    //               // this.loaderEnabled = true
    //             } else {
    //               this.testCollection.testStatus = 1
    ...
    //               this.testOngoing = false
    //               // this.loaderEnabled = true
    //               // this.$router.push({ name: 'ScreeningTests', params: { role: this.$store.getters.userRole } })
    //             }
    //           }).catch((error) => {
    //             console.error(error.message)
    //           }))
    ...
    updateTestCollection () {
      // firestore.collection('users').doc('candidate')
      //   .collection(this.uid).doc('tests')
      //   .collection(this.testId).doc('status').set({
      //     testStatus: 1
    ...
    answerChanged (item) {
      // this.$nextTick(() => {
      //   this.testQuestion.answer.quickReply = item
      // })
    ...
    loadQuestion () {
      // firestore.collection('users').doc('candidate').collection(this.uid).doc('tests').collection(this.testId).doc('data')
      //   .collection('question_and_answers').where('answer', '==', null).orderBy('qno').limit(1).get()
      //   .then((doc) => {
      ...
      //         .then((docsFinished) => {
      //           console.log('FINAL DOC , ', docsFinished)
    ...
    loadTestCollection () {
    //   firestore.collection('candidate_tests').doc(this.testId).get()
    //     .then((gDoc) => {
    //       firestore.collection('users').doc('candidate').collection(this.uid).doc('tests').collection(this.testId)
    //         .get().then((cDoc) => {
    //           const testCol = {
